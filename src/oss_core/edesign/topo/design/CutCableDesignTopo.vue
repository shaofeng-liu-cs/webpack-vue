<template>
    <div class="topo-content">
        <TopoView ref="cutCableTopo" :toolButtons="toolButtons" :show-toolbar="true" :graphView="graphView" :menu-source="initMenus" @click-toolbar="toolbarClick"></TopoView>
...
export default {
    ...
    methods: {
        loadTopo() {
            this.topoId = 'CUT-CABLE-DESIGN' + this.params.resId;
            ...
            let p2 = loadCutCable(params);
            let deviceParam = {
                resId: this.topoResInfo.deviceList[0].resId,
                resType: this.topoResInfo.deviceList[0].resSpecId,
                cableId: this.params.resId,
            };
            let p3 = loadBuildingDevice(deviceParam);
            Promise.all([p0, p1, p2, p3]).then(datas => {
                let topoData = datas[0];
                this.cableInfo = datas[1].resultList[0];
                let topoResData = datas[2];
                this.buildingDeviceList = datas[3];
                if (this.buildingDeviceList.length > 0) {
                    this.isInnerDevice = true;
                }
                this.drawCableTopo(this.cableInfo, topoResData, topoData);
            });
        },

        dealDevicePointResList(pointResList) {
            let newPointResList = pointResList;
            this.buildingDeviceList.forEach(res => {
                let resList = pointResList.filter(item => item.resId === res.resId);
                let count = resList.length;
                if (count === 0) {
                    res.resType = res.resSpecId;
                    res.resName = res.name;
                    res.resNo = res.fullName;
                    res.cutUnitRecord = res.cutUnitRecord || null;
                    newPointResList.push(res);
                }
            });
            return newPointResList;
        },
        ...
        drawCableTopo(cableInfo, topoResData, topoData) {
            let logicCableList = topoResData.logicCableList || [];
            ...
            pointResList = this.dealDevicePointResList(pointResList);

            this.dealLogicListData(pointResList);
