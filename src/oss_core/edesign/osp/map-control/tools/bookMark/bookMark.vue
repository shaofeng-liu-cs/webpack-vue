<template>
  <Popup :popupObj="popupBook" @popupOther='popupOther' @popupOk="popupOk" @popupClose="popupClose">
    <template v-slot:popupBody>
      <el-table :data='bookData' ref="gridBook" id="gridBoodID" @row-dblclick="dbClick" height="300px">
        <el-table-column :label="$t('OSP_MAP_SEARCHBAR_COLOUMN_NAME')" prop="bookmarkName" min-width="120">
          <template slot-scope="scope">
            <el-input v-model="scope.row.bookmarkName" :placeholder="$t('PLEASE_ENTER')" size="mini">
            </el-input>
          </template>
        </el-table-column>
        <el-table-column :label="$t('OSP_MAP_SEARCHBAR_CREATE_TIME')" prop="createDate" min-width="150">
        </el-table-column>
  ...
  methods: {
    ...
    deleteBookMark(row) {
      fish.confirm(this.$t('OSP_MAP_TOOLS_BOOKMARK_SURE_DELETE'), () => {
        let param = {};
        param.bookmarkList = this.bookData;
