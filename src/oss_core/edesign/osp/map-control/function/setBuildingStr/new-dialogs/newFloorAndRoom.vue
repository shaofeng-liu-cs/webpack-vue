<template>
    <Popup :popupObj="popupNew" @popupClose="popupClose" @popupOk="popupOk">
        <template v-slot:popupBody>
            <div>
                <div>
                    <h5>{{ $t('MAP_CON_FUNC_FLOOR_INFO') }}</h5>
                    <form class="contentForm" ref="floorInfo">
                        <div>
                            <div class="content col-md-6">
                                <div class="col-md-5 inputLabel required-label">
                                    <label class="control-label">{{ $t('MAP_CON_FUNC_NAME_PREFIX') }}</label>
                                </div>
                                <div class="col-md-7">
                                    <input type="text" class="form-control" v-model="row.floorPrefix" name="prefix" data-rule="required" />
                                </div>
                            </div>
                            <div class="content col-md-6">
                                <div class="col-md-5 inputLabel required-label">
                                    <label class="control-label">{{ $t('MAP_CON_FUNC_FLOOR_QUANTITY') }}</label>
                                </div>
                                <div class="col-md-7">
                                    <input type="number" class="form-control" v-model="row.floorCount" name="floorCount" data-rule="required" />
                                </div>
                            </div>
                        </div>
                        <div>
                            <div class="content col-md-6">
                                <div class="col-md-5 inputLabel">
                                    <label class="control-label">{{ $t('MAP_CON_FUNC_HEIGHT') }}</label>
                                </div>
                                <div class="col-md-7">
                                    <input type="text" class="form-control" v-model="row.floorHeight" name="floorHeight" data-rule="digits;" />
                                </div>
                            </div>
                        </div>
                    </form>
...
export default {
    ...
    data() {
        return {
            popupNew: {
                title: this.$t('OSP_MAP_FUNCTION_NEW_FLOOR_TITLE'), // 弹框标题
                ok: this.$t('OK'), // 确认按钮内容
    ...
    methods: {
        ...
        popupOk() {
            this.$initForm = $(this.$refs.floorInfo).form();
            ...
            this.$emit('addFloorAndRoom', this.row);
            this.popupNew.show = false;
            this.row = {};
        },
    },
