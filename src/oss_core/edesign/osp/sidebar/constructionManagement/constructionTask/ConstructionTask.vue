      ...
      <el-scrollbar>
        <div class="task-content" v-for="(items,indexs) in taskList" :key="indexs" :class="{active:selectedResID===items.resID}" @click="selectRes(items.resID)">
          <div class="task-head">
            ...
            <div class="task-new" v-if="items.taskList.length === 0" @click="launch(items.resID)">{{$t('LAUNCH')}}</div>
          </div>
          <div class="task-body">
...
<script>
import { queryBuildTask, queryResourcelist } from '@/oss_core/edesign/api/query/schema.js';
  ...
  methods: {
    ...
    selectRes(id) {
      ...
      this.$map.getByFeatureLayers(featureLikeList).then((child) => {
        ...
        // this.mapBox.centerGeometries([child[0].geometry], this.initzoom + 2);
        // let fillSymbol = new this.$map.CompositeSymbol([
        //   {
        //     style: Color.selectedBorderLine,
        //   },
        //   {
        //     style: Color.selectedLine,
        //   },
        // ]);
        // selectGeo[0] && this.mapBox.addGeometry(selectGeo[0].id, selectGeo[0].geometry, fillSymbol, {}, this.mapBox._selectLayer.layerNo);
    ...
    initTask() {
      ...
      queryBuildTask(param).then((data) => {
