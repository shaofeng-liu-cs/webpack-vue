    ...
    <div class="layer-control-restap">
      ...
      <Popover placement="bottom" trigger="click" ref="popoverF" popClass="layer-control-popover">
        ...
        <ul slot="content" class="v9-popover-ul">
          ...
          <li @click="checkTap('other', $t('LAYER_CONTROL_RESOURCE_OTHER'))" class="btn btn-default filter-li">{{$t('LAYER_CONTROL_RESOURCE_OTHER')}} </li>
        ...
        <el-scrollbar>
          ...
          <div v-for="(items, indexs) in gridData" :key="indexs" v-show="items.dataLength !== 0 && items.tapType === resTap">
            <div class="layer-grid-tap-tit">
              <div class="tap icon_osp icon_ospdown" :class="{icon_ospright: !defShow}" @click="gridToggle(`tree-${indexs}`, $event)"></div>
              <div>{{ items.name }}</div>
              <div></div>
              <div></div>
              <div>
            ...
            <div class="layer-grid-body-content" :ref="`tree-${indexs}`" v-show="defShow">
              <div v-for="(item, index) in items.childData" :key="index">
                <div class="layer-grid-tap-tit layer-grid-tap-content bottom-line" v-show="item.isShow">
                  <div>
                    <div></div>
                    <div class="tap icon_osp icon_ospdown" :class="{icon_ospright: !defShow}" @click="gridToggle(`tree-${indexs}-${index}`, $event)"></div>
                    ...
                    <div>
                      <img alt="" v-if="resTap === 'resource' && item.style" :src="item.style.iconUrl" style="cursor: pointer;">
                      <img v-else-if="resTap === 'other'" :src="`oss_core/edesign/assets/image/map/res-other/${item.resType}.svg`" alt="">
                  ...
                  <div :ref="`tree-${indexs}-${index}`" v-show="defShow">
                    <div class="layer-grid-theme-tap-tit" v-for="(attr, attrIndex) in item.gisLayerThemeList" :key="attrIndex">
                      <div>
                        <div class="tap icon_osp icon_ospdown" :class="{icon_ospright: !defShow}" @click="gridToggle(`tree-${indexs}-${index}-${attrIndex}`,$event)"></div>
                      ...
                      <div :ref="`tree-${indexs}-${index}-${attrIndex}`" v-show="defShow">
                        <div v-for="(themeChild, themeChildIndex) in attr.gisLayerThemeItemList" :key="themeChildIndex">
                          ...
                          <img class="theme-img" :src="themeChild.iconUrl" alt="" v-if="themeChild.iconUrl" @click="iconStyleClick(item,themeChild)">
...
import { initRouteObj } from '../../domain/ResEventHandler';
// import { addNewHLDTemplateInfo } from '@/oss_core/edesign/api/add/resources.js';
// const realResourceServiceNoList = [
//     'edesign.graphic-layer-service-geoserver',
//     'edesign.dynamic-layer-service-geoserver',
//     'edesign.other-design-layer-service-geoserver',
// ];
export default {
  name: 'LayerControl',
  inject: ['getProjectInfo', 'getResTypeConfig', 'getLayerCheckData', 'getSurfaceLayerList', 'getMapBox', 'getHldEl'],
  computed: {
    resourceList() {
      let resourceList = this.getResTypeConfig(); // 资源图层
      let surfaceLayerList = this.getSurfaceLayerList().map(res => {
        res.flag = 'surface';
        return res;
      }); // 地貌图层
  ...
  data() {
    return {
      ...
      filterCode: 'resource',
      allResTypeList: [], //资源库的所有图层
      defShow: false, //默认展开还是关闭
    };
  },
  created() {
  ...
  mounted() {
    queryAllrestypelist(this.getProjectInfo().mapNo).then(data => {
      let allResTypeList = data.resultList || [];
      let surfaceLayerList = this.getSurfaceLayerList();
      // 合并：mapNo下所有的资源类型和地貌类型，手动再加一个route，route不在后台配置，但也算作资源
      this.allResTypeList = allResTypeList.concat(surfaceLayerList);
      this.allResTypeList.push(initRouteObj(this.getProjectInfo()));
      this.getGridData();
    });
  },
  methods: {
    ...
    getGridData() {
      ...
      // 没有配置的数据判断是否是设计准备，设计准备只展示地貌跟规划网格，设计中默认设置为勾选，但不展示地貌跟其他设计的
      newResData.forEach(res => {
        if (!res.isSet) {
          let flag = false;
          if (this.projectInfo.markFlag === 'sprint') {
            if (res.surfaceCat || res.layerNo === 'PLAN_GRID-C') {
              flag = true;
            }
          } else if (!(res.layerType === 'R' || res.surfaceCat)) {
            flag = true;
          }
          this.$set(res, 'isVisible', flag);
          this.$set(res, 'isLabelVisible', false);
          this.$set(res, 'isSelectable', flag);
          this.$set(res, 'isSnap', flag);
        }
        if (res.gisLayerThemeList && res.gisLayerThemeList.length > 0) {
          res.gisLayerThemeList.forEach(themeList => {
            ...
            themeList.gisLayerThemeItemList.forEach(theme => {
              let flag = this.projectInfo.markFlag === 'sprint' ? 'N' : 'Y';
              if (!theme.isSet) {
                this.$set(theme, 'isVisible', flag);
                theme.isLabelVisible = flag;
                theme.isSelectable = flag;
                theme.isSnap = flag;
              }
      ...
      let otherSurfaces = allMapLayers.filter(all => !this.allResTypeList.some(res => res.layerNo === all.id));
      if (otherSurfaces && otherSurfaces.length > 0) {
        ...
        otherSurfaces.forEach(other => {
          ...
          let findRes = newCheckData.find(res => res.layerNo === other.id);
          ...
          let obj = {
            ...
            isLabelVisible: findRes ? findRes.isLabelVisible === 'Y' : false,
            isSelectable: findRes ? findRes.isSelectable === 'Y' : false,
            isShow: true, // 是否在列表中展示
            isSnap: findRes ? findRes.isSnap === 'Y' : false,
            isVisible: findRes ? findRes.isVisible === 'Y' : false,
    ...
    checkboxOne(name, items, checked, item, isClick, themeChildIndex) {
      // 单选的联动操作
      let check = checked;
      item[name] = check;
      if (name === 'isVisible') {
        ...
        if (!check && item.isLabelVisible !== check) {
        ...
        if (themeChildIndex === undefined) {
          this.delCheckboxThemeAll(item, check);
        }
      } else {
    ...
    //查找当前资源类型的所有数据
    searchRes(resInfo) {
      // let resType = resInfo.resType + '-' + resInfo.resStatus;
      let Filter = this.$map.Filter;
      let filterKey = resInfo.resType === 'A5' ? 'sprintId' : 'schemaId';
      // let filter = Filter.and(
      //     Filter.equalTo({
      //         propertyName: filterKey,
      //         expression: this.getProjectInfo()[filterKey],
      //     }),
      //     Filter.equalTo({
      ...
      //     })
      // );
      // if (resInfo.resType === 'A5') {
      //   resType = resInfo.layerNo;
      // }
      let filter = Filter.equalTo({ propertyName: filterKey, expression: this.getProjectInfo()[filterKey] });
      let featureLikeList = [
        {
          layerNo: resInfo.layerNo,
          filter: filter,
        },
      ];
      this.$map.getByFeatureLayers(featureLikeList).then(data => {
        if (data && data.length !== 0) {
          data.forEach(item => {
            item.resType = resInfo.resType;
            item.resTypeName = resInfo.resTypeName;
          });
          this.$root.$emit('property', data);
        } else {
          fish.toast('warn', this.$t('LAYER_DATE_NULL'));
        }
      });
    },
    ...
    // 单个主题全选 type是否需要全选下一级主题  界面点击事件触发需要, 其他情况不需要传true
    checkboxTheme(name, item, isCheck, items, attrIndex, isClick) {
    ...
    // 主题全部取消
    delCheckboxThemeAll(item, check) {
      if (item.gisLayerThemeList) {
        item.gisLayerThemeList.forEach(attr => {
          attr.isVisible = check ? 'Y' : 'N';
          attr.gisLayerThemeItemList.forEach(themeChild => {
            themeChild.isVisible = check ? 'Y' : 'N';
          });
        });
      }
    },
    ...
    checkboxThemeOne(attr, isCheck, themeChildIndex, items, item, attrIndex, isClick) {
      ...
      if (isCheck === 'N') {
        if (attr.isVisible === 'Y') {
          this.checkboxTheme('isVisible', item, 'N', items, attrIndex, false);
        }
      } else {
        let someN = attr.gisLayerThemeItemList.some(themeChild => themeChild.isVisible === 'N');
        !item.isVisible && this.checkboxOne('isVisible', items, true, item, false, themeChildIndex);
        if (!someN) {
          attr.isVisible = 'Y';
        }
      }
    ...
    save(refreshLayers) {
      ...
      let param = {
        gisLayerConfigList: this.layerCheckData,
      };
      let url;
      ...
      this.$emit('refreshLayers', false, refreshLayers);
      fish.post({
        url: url,
        data: param,
        success: data => {
    ...
    iconStyleClick(item, style) {
      console.log(item);
      console.log(this.resourceList);
      // console.log('噢噢噢噢：', JSON.stringify(obj));
      fish.require(['oss_core/edesign/hldTemplate/views/SettingStyle'], aDialog => {
        let options = {
          type: 'new',
          resType: item.resType,
          styleJson: style,
        };
        console.log('啥：', JSON.parse(JSON.stringify(options)));
        ...
        fish.listenTo(this.dialogView, 'OKEvent', result => {
          ...
          let layer = layers.find(layer => layer.id === item.layerNo);
          this.$emit('setStyle', layer, item.style);
          this.mapBox.refreshLayers([item.layerNo]);
          //   addNewHLDTemplateInfo
        });
      });
    },
  },
