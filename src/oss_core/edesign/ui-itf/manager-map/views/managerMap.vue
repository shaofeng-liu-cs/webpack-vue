...
<template>
    <div id="app" class="box">
        <keep-alive>
            <Map v-if="detail.sprintId" ref="hldDesign" :queryData="detail" @openMenuInHld="openMenu" @btnToCoverCurrentPage="btnToCoverCurrentPage"></Map>
        </keep-alive>
    </div>
</template>
<script>
import { loadSingleSprintList } from '../../../osp/task/api/taskAction';
import Map from './designMap.vue';
...
export default {
    ...
    mounted() {},
    created() {
        let sprintId = this.initData.schemaId ? this.initData.schemaId.toString() : '';
        loadSingleSprintList(sprintId).then(result => {
            if (result && result.resultList && result.resultList.length === 1) {
                result.resultList[0].sourceFlag = 'manager-map';
                result.resultList[0].markFlag = 'sprint';
                result.resultList[0].schemaId = result.resultList[0].sprintId;
                this.detail = result.resultList[0];
            } else {
