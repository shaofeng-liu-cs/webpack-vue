<template>
    <div class="bom">
        ...
        <div class="right">
            ...
            <div class="exportBtn">
                <download-excel class="export-excel-wrapper" :data="downData" :fields="downCol" type="xls" :name="fileName">
                    <!-- 上面可以自定义自己的样式，还可以引用其他组件button -->
                    <button type="button" class="btn btn-primary" @click="popupOk">{{$t('EXPORT')}}</button>
                </download-excel>
            </div>
        </div>
    </div>
...
<script>
import { getConfigExtend, queryBomInfoBySchemaId, queryBomTypeBySchemaId } from '@/oss_core/edesign/api/query/schema';
...
export default {
  ...
  mounted() {
    ...
    this.fileName = 'BOM_' + this.projectInfo.name + '.xls';
  },
  ...
  methods: {
    ...
    exportA(fileName, grid) {
      ...
      fileName = '' + this.projectInfo.name + '_' + '_BOM_' + date + '_' + this.add0(this.seq, 'S');

      this.downloadExl(newData, 'New', fileName + '.xls', '');
      this.seq++;
    },
