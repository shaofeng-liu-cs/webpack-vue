...
function addDemandPoint(dataInfo, mapBox, projectInfo, that) {
  ...
  getNearestPoint(dataInfo, mapBox).then(result => {
    ...
    map.getByFeatureLayer(featureLayer).then(childResult => {
...
function buildingTransfer(dataInfo, mapBox, that) {
  const { CloudGraphic } = dataInfo;
  const fillSymbol = new that.$map.CompositeSymbol([
    {
      style: CloudGraphic.geometry.geometryType === 'geometryMultipolygon' ? Colors.fillPolygonColor : Colors.mouseStyle,
      geometry: null,
    },
    {
  ...
  const newGra = mapBox.addGeometry(CloudGraphic.id, CloudGraphic.geometry, fillSymbol, CloudGraphic.attributes, 'demandBuilding');
  mapBox.refresh();
  that.$emit('map-selection', [newGra]);
}
...
export function resFunction(functionData, dataInfo, mapBox, projectInfo, surfaceList, that) {
  ...
  switch (functionData.url) {
    ...
    case 'resReuse':
      ...
      break;
    case 'buildingTransfer':
      buildingTransfer(dataInfo, mapBox, that);
      break;
    default:
