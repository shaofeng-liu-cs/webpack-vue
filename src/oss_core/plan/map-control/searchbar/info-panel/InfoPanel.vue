<template>
    <div class="info-panel" ref="draggable" v-if="controlData.popupName === 'infoPanel'">
        <div class="search-info">
            <div class="info-head">
                <div>
                    <div :class="{ active: contentType === 'RESOURCE', disabled: !resShow }" data-name="RESOURCE" @click="contentTap">{{ $t('RESOURCE') }}</div>
                    <div :class="{ active: contentType === 'GIS', disabled: !gisShow }" data-name="GIS" @click="contentTap">{{ $t('OSP_MAP_SEARCHBAR_ITEM_GIS') }}</div>
                    <div :class="{ active: contentType === 'BUILDING'}" v-if="isDPModel" data-name="BUILDING" @click="contentTap">{{ $t('OSP_MAP_SEARCHBAR_ITEM_BUILDING') }}</div>
            ...
            <div class="info-content" v-if="gisShow && contentType === 'GIS'">
                <div>
                    ...
                    <div class="info-name" :title="dataInfo.CloudGraphic.attributes.resName">
                        <div class="name-div">{{ dataInfo.CloudGraphic.attributes.resName}}</div>
                    </div>
                    <!-- <div class="info-save" @click="save(dataInfo.CloudGraphic)" :class="projectInfo.markFlag === 'sprint' ? 'notClick' : ''">{{ $t('SAVE') }}</div> -->
                </div>
                <div class="info-form">
                    ...
                    <form class="form-horizontal" ref="saveForm">
                        <div v-for="(attr, index) in attrInfoList" :key="index + attr.attrName" :style="{ order: attr.dispSeq }">
                            ...
                            <input class="form-control" :name="attr.attrName" v-model="attributes[attr.attrName]" :placeholder="attr.controlFlag === '2' ? '' : 'Please input content'" v-if="attr.controlType === '1'" :disabled="attr.disabled" :type="attr.dataType === 'N' ? 'number' : 'text'" />
                            <input class="form-control" :name="attr.attrName" v-model="attributes[attr.attrName]" :placeholder="attr.controlFlag === '2' ? '' : 'date'" v-else-if="attr.controlType === '2'" :disabled="attr.disabled" :type="attr.dataType === 'N' ? 'number' : 'text'" />
                            <Combobox :name="attr.attrName" v-model="attributes[attr.attrName]" :setting="{ dataTextField: 'valueDesc', dataValueField: 'attrValue', dataSource: attr.attrValuesDTOS, value: attr.initValue }" v-else-if="attr.controlType === '3'" :disabled="attr.disabled"></Combobox>
                        </div>
                    </form>
                </div>
            </div>
            <div class="info-content" v-if="contentType === 'BUILDING'">
                <div>
                    ...
                    <div class="info-name" :title="buildingDataInfo.CloudGraphic.attributes.resName">
                        {{ buildingDataInfo.CloudGraphic.attributes.resName }}
                    </div>
                    <!-- <div class="info-save" @click="save(dataInfo.CloudGraphic)" :class="projectInfo.markFlag === 'sprint' ? 'notClick' : ''">{{ $t('SAVE') }}</div> -->
                </div>
                <div class="info-form">
        ...
        <!-- <Function :params="functionParams" ref="funcitonView" @changeAttributes="changeAttributes" v-on="$listeners" v-if="['GIS', 'RESOURCE', 'BUILDING'].indexOf(contentType) > -1"></Function> -->
    </div>
</template>

<script>
import Function from '@/oss_core/plan/map-control/function/Function.vue';
...
export default {
  ...
  mixins: [ResHandler],
  inject: ['getMapBox', 'getResTypeAttrDtos', 'getControlData', 'getSurfaceLayerList'],
  computed: {
    mapBox() {
  ...
  methods: {
    // 初始化函数
    queryAttrInfo(item, back, number, clickType) {
    ...
    // save(CloudGraphic) {
    //     this.$map.updateGeometryById(CloudGraphic.layerNo, CloudGraphic).then(data => {
    //         this.dataInfo.CloudGraphic.attributes = this.attributes;
    //         this.mapBox.refreshLayer(CloudGraphic.layerNo);
    ...
    moreInfo() {
      // this.$emit('property', [this.dataInfo.CloudGraphic]);
    },
    centerResult() {
