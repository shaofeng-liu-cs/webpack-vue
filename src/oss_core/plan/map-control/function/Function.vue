 ...
 * @LastEditTime: 2022-11-17 20:48:54
  ...
  mounted() {
    ...
    if (this.dataInfo.CloudGraphic.resType === 'A5') {
      this.$map
        ...
        .then(res => {
          this.hasModel = !!res.attributes.resId;
  ...
  methods: {
      ...
      queryFunctionInfo(params).then(data => {
        if (data && data.resultList && data.resultList.length !== 0) {
          ...
          data.resultList.forEach(item => {
            ...
            item.isEnable = this.setIsTrue(item, item.enableControl, sourceFlag, beRefFlag);
          });
    ...
    setIsTrue(item, type, sourceFlag, beRefFlag) {
      ...
      if (graph.resType === 'A5') {
        ...
        hasModel = !!graph.attributes.resId;
      }
      if (
        (list[0] === 'Y' && !isReuse) ||
        (list[0] === 'N' && isReuse) ||
          ...
          if (item.viewType === '1') {
            ...
            let params = {
              portParams: {
                ...
                resType: this.dataInfo.CloudGraphic.relaResType ? this.dataInfo.CloudGraphic.relaResType : this.dataInfo.CloudGraphic.resType,
